...
func (m *Model) GetServerSettings() (sms *ServerModelSettings) {
	modelName := CamelCase(m.Name)
	sms = &ServerModelSettings{
		idCol:              fmt.Sprintf("%sId", modelName),
		indexFunc:          "GetAll",
		getFunc:            "GetById",
		createFunc:         "Create",
		updateFunc:         "Update",
		...
		indexRoute:         fmt.Sprintf("/%s", modelName),
		getRoute:           fmt.Sprintf("/%s/:%s", m.Name, fmt.Sprintf("%sId", modelName)),
		createRoute:        fmt.Sprintf("/%s", modelName),
		updateRoute:        fmt.Sprintf("/%s", modelName),
		deleteRoute:        fmt.Sprintf("/%s/:%s", modelName, fmt.Sprintf("%sId", modelName)),
		controllerVar:      fmt.Sprintf("%sController", m.Name),
		controllerName:     fmt.Sprintf("%sController", strings.Title(m.Name)),
		controllerFileName: fmt.Sprintf("%sController.go", modelName),
		modelName:          fmt.Sprintf("%sModel", strings.Title(m.Name)),
		modelFileName:      fmt.Sprintf("%sModel.go", modelName),
	}
	for _, fld := range m.Fields {
		sms.scanParams += fmt.Sprintf("&%s.%s,", modelName, strings.Title(fld.Name))
		sms.selectList += fmt.Sprintf("%s,", fld.Name)
		if fld.AutoGenerated && fld.Type == Integer { //leave id column
			continue
		}
		sms.updateParams += fmt.Sprintf("%s.%s,", modelName, strings.Title(fld.Name))
		sms.updateList += fmt.Sprintf("%s=?,", fld.Name)
		sms.insertList += fmt.Sprintf("%s,", fld.Name)
		sms.placeholderList += "?,"
	}
	//set the DB access params
